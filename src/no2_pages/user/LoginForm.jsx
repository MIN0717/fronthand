...
const LoginForm = ({users,setLoginMode}) => {
  ...
  return (
    <Form onSubmit={handleSubmit}>
        <Title>로그인</Title>
        <Card>
            <Input type="text" name='username' value={user.username} onChange={handleChange} placeholder='아이디'/>
        </Card>
        <Card>
             <Input type="password" name='password' value={user.password} onChange={handleChange} placeholder='비밀번호' />
        </Card>
        <Card>
            <LoginButton>로그안</LoginButton>
            <RegisterButton onClick={()=>navigate("/register")}>아직 회원이 아닌신가요? 회원가입</RegisterButton>
